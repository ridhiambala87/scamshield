...
function validateAndNormalizeOutput(parsed, localFallback) {
  ...
  // Preserve hard deterministic signals even when the AI labels content LOW —
  // a typosquat/impersonation CRITICAL can never be talked down to a pass.
  const hardLocal = (localFallback.threat_signals || []).filter(s => s.severity === 'CRITICAL' || s.severity === 'HIGH');
  let threatSignals = (level === 'LOW') ? [] : (Array.isArray(parsed.threat_signals) && parsed.threat_signals.length ? parsed.threat_signals : localFallback.threat_signals);

  if (hardLocal.length > 0 && (level === 'LOW' || score < 25)) {
    level = 'HIGH';
    score = Math.max(score, 65);
    threatSignals = hardLocal;
  }

  // Always merge deterministic hard signals the AI failed to report (dedupe by label),
  // so the evidence chain always shows e.g. the rn→m typosquat CRITICAL.
  if (hardLocal.length > 0) {
    const have = new Set(threatSignals.map(s => (s.type || '') + '|' + (s.label || '')));
    for (const h of hardLocal) {
      const key = (h.type || '') + '|' + (h.label || '');
      if (!have.has(key)) { threatSignals.push(h); have.add(key); }
    }
  }

  const threatBreakdown = (threatSignals.length === 0) ? {
